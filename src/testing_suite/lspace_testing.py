...
from src.classifiers.model import ResNet18_CBAM
from src.classifiers.model import ResNet50_CBAM
...
def run_evaluation(args: argparse.Namespace) -> None:
    ...
    if KernelInceptionDistance is not None:
        try:
            sig = inspect.signature(KernelInceptionDistance)
            kwargs = {"subset_size": 1000}
            if "subset_replacement" in sig.parameters:
                kwargs["subset_replacement"] = False
            kid_metric = KernelInceptionDistance(**kwargs).to(device)
        except Exception as err:
            print(f"Warning: failed to initialize KID metric: {err}")
            kid_metric = None
    else:
